...
class MVPDataPlatformScanner:
    ...
    def _metric_input_for(self, metric: str, ctx: Dict[str, Any]) -> Any:
        ...
        # If there are dependency results available in ctx (from prior waves), attach only
        # the declared dependencies' outputs to the input under `dependency_results`.
        dep_results = {}
        ...
        if dep_results:
            # ensure we return a dict that includes both the original/base input and dependency outputs
            if isinstance(base, dict):
                # don't overwrite keys — put dependency outputs under a dedicated key
                result = dict(base)
                # avoid clobbering existing key named "dependency_results"
                if "dependency_results" in result:
                    # nest under _dependency_results to be safe
                ...
                # base is not a dict (e.g., a list or None) — create a wrapper
                return {"input": base, "dependency_results": dep_results}

        # no dependency outputs to attach — return base as-is
        return base

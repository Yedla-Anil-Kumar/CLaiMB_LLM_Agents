...
"""
Metric Prompt Builder (deps-aware, lean)
- Universal preamble + unified response format (your existing contract)
- Adds a small, generic Dependencies section when task_input["deps"] exists
- Keeps tokens low: short hints only; relies on actual numbers in task_input
"""
...
UNIVERSAL_PREAMBLE = (
    ...
    "- Use only provided data. If required inputs are missing, put those as numbered items inside 'gaps', reduce 'confidence', and adjust the score downward.\n"
    "- Prefer normalized rates (0..1), p95/p99, and denominators. Cite exact numbers under 'evidence'.\n"
    "- If a 'deps' object is present in TASK INPUT, TREAT IT AS AUTHORITATIVE upstream metric outputs and use its numbers directly.\n"
    "- Keep recommended next steps concrete (≤5) and put them INSIDE 'gaps' as numbered items (no 'actions' field).\n"
    "- Return ONLY the specified JSON. No extra text.\n"
    "- Do NOT reuse numbers from EXAMPLE OUTPUT; recompute everything from TASK INPUT."
)

# Actions are embedded within 'gaps'
UNIVERSAL_RESPONSE_FORMAT = (
    '{"metric_id":"<id>",'
    '"score":<1-5>,'
    '"rationale":"<2-4 sentences, human-readable>",'
...
DEPENDENCY_HINTS = {
    "tagging.coverage": (
        "Upstream tagging quality. Key fields:\n"
        "- evidence.coverage_pct: fraction (0..1) of fully tagged resources.\n"
        "- evidence.missing_examples[]: concrete items with absent critical tags "
        "(env, owner, cost-center, service)."
    ),
    "compute.utilization": (
        "Fleet/instance efficiency. Key fields:\n"
        "- evidence.idle_pct: share (0..1) of fleet considered idle/low-util.\n"
        "- evidence.worst_idle_hours: max low-util hours observed on a single instance.\n"
        "- evidence.fleet_cpu_p95 / evidence.fleet_mem_p95: overall pressure."
    ),
    "k8s.utilization": (
        "Cluster packing & scheduling efficiency. Key fields:\n"
        "- evidence.binpack_efficiency: packing/fragmentation (0..1; higher is better).\n"
        "- evidence.pending_pods_p95: backlog pressure.\n"
        "- evidence.pods_cpu_req_vs_used: request vs actual usage alignment (0..1)."
    ),
}

METRIC_DEP_USAGE_GUIDE = {
    "cost.allocation_quality": (
        "How to use deps:\n"
        "- Treat deps['tagging.coverage'].evidence.coverage_pct as an UPPER BOUND on the "
        "cost fraction that can be attributed via tags.\n"
        "- Compute attributable_cost from params.cost_rows by checking presence of critical tags "
        "(env, owner; include cost-center/service if your policy requires). Then bound it by "
        "coverage_pct * total_cost to avoid over-credit when fleet tagging is weak.\n"
        "- If evidence.missing_examples contains prod resources missing owner/cost-center/service, "
        "penalize the score and list those gaps.\n"
        "Evidence to return: {'attributable_pct': <0..1>, 'coverage_pct': <0..1>, "
        "'missing_examples': <n>}"
    ),

    "cost.idle_underutilized": (
        "How to use deps:\n"
        "- Prefer direct per-instance signals in params.instances. Mark a row idle if both "
        "cpu_p95 and mem_p95 are below your low-util threshold (e.g., 0.30). Sum its cost into idle_cost.\n"
        "- If instance-level utilization is incomplete, fallback to deps['compute.utilization'].evidence.idle_pct "
        "to estimate idle_cost = idle_pct * total_cost.\n"
        "- Optionally, corroborate with deps['k8s.utilization']: if binpack_efficiency < 0.75 or "
        "pending_pods_p95 is high, reduce confidence or increase risk notes; if strong (>0.8 and low pending), "
        "be cautious labeling shared cluster costs as idle.\n"
        "Evidence to return: {'idle_cost': <number>, 'idle_pct': <0..1>, 'total_cost': <number>, "
        "'fleet_cpu_p95': <0..1>, 'fleet_mem_p95': <0..1>, 'binpack_efficiency': <0..1> (if present)}"
    ),

    "scaling.effectiveness": (
        "How to use deps:\n"
        "- Compute all primary signals from params.ts_metrics and params.scale_events (reaction time, "
        "violation %, thrash, delta adequacy).\n"
        "- Use deps['compute.utilization'] only as context: if evidence.idle_pct is high, be skeptical of scale_out; "
        "if fleet_cpu_p95 is high during breaches, it supports the need to scale. Do NOT replace time-series "
        "calculations with deps values.\n"
        "Evidence to return: {'median_reaction_s': <sec>, 'target_violation_pct': <pct>, 'thrash_rate': <pct>, "
        "'delta_error_pct': <pct>, 'events': <n>, 'total_samples': <n>}"
    ),
}
...
def build_prompt(metric_id: str, task_input: dict) -> str:
    ...
    # 1) Build SYSTEM: use your stored system prompt, but ensure it contains the universal preamble/rules
    # (If your stored meta['system'] already incorporates UNIVERSAL_PREAMBLE + rubric + APPEND_TO_ALL_METRICS, no merge needed.)
    system_block = meta["system"]

    # 2) Friendly meanings for input keys (optional but useful)
    meanings = meta.get("input_key_meanings", {}) or {}
    key_meanings_str = "\n".join([f"- {k}: {v}" for k, v in meanings.items()]) if meanings else ""

    # 3) Dependencies section (only if deps exist)
    deps = (task_input or {}).get("deps", {}) or {}
    if deps:
        dep_lines = []
        for dep_key in deps.keys():
            # short hint for each upstream metric
            hint = DEPENDENCY_HINTS.get(dep_key, "Upstream metric output; rely on its evidence fields.")
            dep_lines.append(f"- deps['{dep_key}']: {hint}")
        usage = METRIC_DEP_USAGE_GUIDE.get(metric_id)
        deps_section = "DEPENDENCIES (from prior metrics):\n" + "\n".join(dep_lines)
        if usage:
            deps_section += f"\n\nDEPENDENCY USAGE GUIDE (for {metric_id}):\n{usage}"
        deps_section += "\n\n"
    else:
        deps_section = ""
    ...
    # Optional debugging:
    # logger.debug(f"Prompt for {metric_id}:\n{prompt}")
    return prompt
